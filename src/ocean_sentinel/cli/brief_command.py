...
def get_ais_proximity_audit(site_id: str) -> dict:
    """Return per-chunk AIS ground-truth audit for this site, sourced from
    MarineCadastre.gov (NOAA Office for Coastal Management). Each chunk has a
    nearest-vessel CPA distance + named vessel attribution within ±2 min.
    Currently available for: oc01.

    Args:
        site_id: Hydrophone identifier (e.g. 'oc01').
    """
    import json
    from pathlib import Path
    from collections import Counter
    audit_path = Path("data/audit") / f"{site_id}_mc_track.json"
    if not audit_path.exists():
        return {
            "site_id": site_id,
            "audit_available": False,
            "note": (
                "MarineCadastre per-chunk audit not yet run for this site. "
                "Available now for: oc01. Other US-waters SanctSound sites "
                "can be audited via scripts/audit_oc01_marine_cadastre.py "
                "(parameterise bbox + hydrophone coords)."
            ),
        }
    doc = json.loads(audit_path.read_text())
    chunks = doc.get("per_chunk", [])
    n_chunks = len(chunks)

    # Distance band counts
    band_counter: Counter = Counter()
    nearest_names: Counter = Counter()
    for c in chunks:
        nv = c.get("nearest_in_window")
        if not nv:
            band_counter["no_AIS"] += 1
            continue
        d = nv["distance_km"]
        if d <= 5:
            band = "≤5km"
            ...
            band = ">10km"
        band_counter[band] += 1
        name = nv.get("vessel_name") or "(unknown)"
        if name and name != "(unknown)":
            nearest_names[name] += 1

    # Top 3 closest named vessels in the audit window
    closest = sorted(
        doc.get("vessel_cpa", []),
        key=lambda v: v.get("cpa_km", 9999),
    )[:5]

    return {
        "site_id": site_id,
        "audit_available": True,
        "source": doc.get("source", "MarineCadastre.gov"),
        "audit_window_days": doc.get("days", []),
        "n_chunks": n_chunks,
        "distance_bands": dict(band_counter),
        "top_5_nearest_named_vessels": [
            {
                "name": v.get("vessel_name"),
                "mmsi": v.get("mmsi"),
                "cpa_km": round(float(v.get("cpa_km", 0)), 2),
                "cpa_timestamp_utc": v.get("cpa_ts"),
                "vessel_type": v.get("vessel_type"),
            }
            for v in closest
        ],
        "most_frequent_nearest_vessels": nearest_names.most_common(5),
        "labeling_finding": (
            f"Every chunk at {site_id} has a vessel within 10 km in its hour. "
            "Recommend distance-band labels (e.g. ≤5km / 5-7km / 7-10km / >10km) "
            "instead of binary ship/ambient at high-traffic sites."
        ) if not band_counter.get(">10km") and not band_counter.get("no_AIS") else None,
    }
...
Tools available to you:
  ...
  - get_ais_proximity_audit(site_id): MarineCadastre.gov AIS ground truth —
    per-chunk nearest named vessel + CPA distance (gov source, audit-grade)
...
AIS GROUND TRUTH
  [INCLUDE THIS SECTION ONLY IF get_ais_proximity_audit returned audit_available=true.
   Render: per-chunk CPA distribution, top 3 named vessels with exact CPA + UTC
   timestamps from MarineCadastre.gov, plus the labeling_finding string. If
   audit_available=false, OMIT this section header entirely.]
...
SOURCES
  ...
  · AIS ground truth: MarineCadastre.gov (NOAA OCM) — if audit available
...
def brief_command(
    ...
    tools_by_name = {
        ...
        "get_ais_proximity_audit": get_ais_proximity_audit,
    }

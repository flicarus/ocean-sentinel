"""Train OceanSentinelV7.1 — same architecture, better generalization recipe.

What's different vs train_cnn_v7.py
-----------------------------------
1. Focal loss weighting on the evidential MSE+KL — focuses gradient on
   hard samples instead of letting easy ships dominate updates.
2. Lower KL coefficient (default 0.05 vs 1.0). Strong KL collapses
   evidential head to constant ~0.975 confidence; weaker KL leaves room
   for real differentiation.
3. Cross-site ambient mixing in augmentation — every ship sample gets
   randomly mixed with ambient from a *different* site. Forces the
   model to learn vessel signature as "what's consistent across noisy
   backgrounds" rather than memorizing site-specific patterns.
4. Session-level train/val split — adjacent offsets from one recording
   session can no longer leak between train and val. Honest validation.

The architecture (OceanSentinelV7) is unchanged. We're only changing
the training recipe so v7.1 generalizes to unseen hydrophones.

Usage:
    PYTHONPATH=src venv/bin/python scripts/train_cnn_v7_1.py \\
...
def cli() -> None:
    ...
    ap.add_argument("--out", default="data/models/cnn_v7_1.pt")
